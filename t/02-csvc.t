...
my (@train-multi-x, @train-multi-y) := gen-train-multiclass;
my @test-multi-x = 1 => -0.5e0, 2 => 0.5e0;
my @text-multi-y = [4];

{
    ...
    my Algorithm::LibSVM::Parameter $parameter .= new(svm-type => C_SVC,
                                                      kernel-type => LINEAR);
    my Algorithm::LibSVM::Problem $problem = Algorithm::LibSVM::Problem.from-matrix(@train-multi-x, @train-multi-y);
    ok $libsvm.check-parameter($problem, $parameter), { "Given a setting of " ~ $_ ~ ", Algorithm::LibSVM.check-parameter should return True" }("C_SVC/LINEAR");
    my $model = $libsvm.train($problem, $parameter);
    is $model.predict(features => @test-multi-x)<label>, 4.0e0, { "Given a setting of " ~ $_ ~ ", When Algorithm::LibSVM::Model.predict<label> predicts a label of a instance (where the instance is at the center of the cluster labeled as 4 in the training set), it should return 4.0e0" }("C_SVC/LINEAR (Multi Class)");
}
